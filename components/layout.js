          ...
          {home ? (
              ...
              <div className="w-full mx-auto mb-4 flex items-center justify-center space-x-6">
                {data.urls.map((url, index) => (
                  <a href={url.url} key={index} target="_blank">
                    <img
            src = {url.iconPath} className =
                "w-12 h-12 rounded-lg object-cover p-2 hover:shadow-lg hover:shadow-purple-500 transition duration-100" /
                > </a>
                ))}
              </div>
                </>
            ...
            <>
              <Link legacyBehavior href="/">
                 < a>< img
            src = {data.profile} className =
                "rounded-full h-24 w-24 object-cover mt-8 transition hover:border-4 border-purple-500 border-opacity-20 ease-in-out duration-100"
                    alt={
    data.name}
                  />
                </a>
