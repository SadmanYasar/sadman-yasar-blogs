...
function FloatingOrb({ position, color, speed = 1, distort = 0.4, scale = 1 }: FloatingOrbProps) {
...
function getDeterministicPositions(count: number): Float32Array {
  const positions = new Float32Array(count * 3);
  for (let i = 0; i < count; i++) {
    const s1 = Math.sin(i * 12.9898 + 78.233);
    const s2 = Math.sin(i * 45.164 + 92.421);
    const s3 = Math.sin(i * 93.312 + 13.673);
    positions[i * 3] = (s1 - Math.floor(s1) - 0.5) * 50;
    positions[i * 3 + 1] = (s2 - Math.floor(s2) - 0.5) * 50;
    positions[i * 3 + 2] = (s3 - Math.floor(s3) - 0.5) * 50;
  }
  return positions;
}

function ParticleField({ count = 500 }: { count?: number }) {
  const points = useMemo(() => getDeterministicPositions(count), [count]);
